...
void CMiscHacks::Move(CUserCmd *pCmd, bool &bSendPacket)
{
   ...
   /* if (Menu::Window.MiscTab.OtherAutoCounterStafe.GetState())
	...
	if (Menu::Window.RageBotTab.Fakewalk.GetState())
		FakeWalk(pCmd, bSendPacket);

	if (Menu::Window.MiscTab.SlowMotion.GetKey())
		SlowMotion(pCmd);

	
}

static __declspec(naked) void __cdecl Invoke_NET_SetConVar(const char* cvar, const char* value, void* pfn)
{
	__asm 
	{
		push    ebp
...
void DECLSPEC_NOINLINE NET_SetConVar(const char* cvar, const char* value)
{
	...
	if (setaddr != 0) 
	{
		...
		Invoke_NET_SetConVar(cvar, value, pvSetConVar);
	}
}
...
void CMiscHacks::ChatSpamInterwebz()
{
	...
	if (wasSpamming)
	{
		...
		if (useSpace)
		{
			change_name ("\xe1\xab\xa5sensation.cc");
			useSpace = !useSpace;
		}
		else
		{
...
void CMiscHacks::ChatSpamRegular()
{
	...
	if (Menu::Window.MiscTab.OtherTeamChat.GetState())
	{
		SayInTeamChat("Boost service by SENSATION.CC");
	}
	else
	{
		SayInChat("Derank service by SENSATION.CC");
	}
...
void CMiscHacks::SlowMotion(CUserCmd* pCmd)
{
	int num = pCmd->tick_count % 2;
	...
	if (SlowMoKey > 0 && GUI.GetKeyState(SlowMoKey)) {
		...
		if (!pCmd->buttons & IN_ATTACK)
			switch (num) {
			case 0: pCmd->tick_count = 0xFFFFF;
			case 1: break;
			}
	}
}