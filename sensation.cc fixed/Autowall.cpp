...
bool SimulateFireBullet(CBaseCombatWeapon *weapon, FireBulletData &data, IClientEntity *local)
{
	...
	while ((data.penetrate_count > 0) && (data.current_damage >= 1.0f))
	{
			...
			/*&& (DoesRayTouchHitbox(end, &data))*/)
		{
			...
			ScaleDamage(data.enter_trace.m_pEnt, wpn_data->flArmorRatio, data.current_damage, data.enter_trace.hitgroup);
...
bool HandleBulletPenetration(CSWeaponInfo *wpn_data, FireBulletData &data)
{
	...
	float v35 = (data.current_damage * final_damage_modifier) + v34 * 3.0f * (0.0f, (3.0f / wpn_data->flPenetration) * 1.25f);
...
*/
bool CanHit(float *damage_given, const Vector &point)
{
	//Utils::ToLog("CANHIT");
